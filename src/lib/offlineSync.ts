...
import {
  ...
  createRoomCard, updateRoomCard, deleteRoomCard // <--- ADDED: Imported new room card tools
} from './supabase';

// FIXED: Expanded the type to recognize room card actions
export type QueuedOp = {
  id: string;
  type:
    | 'createHotel' | 'updateHotel' | 'deleteHotel'
    | 'createDuration' | 'updateDuration' | 'deleteDuration'
    | 'createRoomCard' | 'updateRoomCard' | 'deleteRoomCard' 
    | 'createEmployee' | 'updateEmployee' | 'deleteEmployee';
  payload: any;
  timestamp: number;
};
...
class OfflineSyncManager {
  ...
  private async execute(op: QueuedOp | Omit<QueuedOp, 'id' | 'timestamp'>) {
    ...
    switch (op.type) {
      ...
      // FIXED: Added execution handlers so Room Cards actually get saved to DB
      case 'createRoomCard': await createRoomCard(p);                            break;
      case 'updateRoomCard': await updateRoomCard(p.id, p);                      break;
      case 'deleteRoomCard': await deleteRoomCard(p.id);                         break;
