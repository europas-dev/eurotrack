...
export default function App() {
  ...
  const handleAddCompany = async () => {
    ...
    if (!error) {
      ...
      setActiveModal(null);
      fetchData(user.id);
      toast.success(language === 'de' ? 'Firma erfolgreich erstellt' : 'Company created successfully');
    }
  };

  // --- ACTION: ADD HOTEL CONTAINER ---
  const handleAddHotel = async () => {
    ...
    if (!error) {
      fetchData(user.id);
      toast.success(language === 'de' ? 'Hotel-Container hinzugefügt' : 'Hotel container added');
    }
  };
  ...
  return (
    <div className={cn("flex h-screen w-full transition-all font-sans", isDarkMode ? "bg-[#020617] text-white" : "bg-slate-50 text-slate-900")}>
      <Toaster position="top-center" richColors />

      {/* SIDEBAR */}
      <aside className={cn("w-80 flex flex-col shrink-0 z-50 border-r", isDarkMode ? "bg-[#0F172A] border-white/5" : "bg-white border-slate-200")}>
        <div className="p-6 flex flex-col h-full">
          <div className="flex items-center gap-2 text-2xl font-black italic mb-10 px-2 cursor-pointer" onClick={() => setView('landing')}>
            <div className="w-8 h-8 bg-blue-600 rounded flex items-center justify-center text-[10px] text-white font-bold">E1</div>
          ...
          <nav className="flex-1 space-y-1 overflow-y-auto scrollbar-hide pr-2">
            {t.months.map((m: string, i: number) => (
              <button key={m} onClick={() => {setActiveMonth(i); setActiveCompanyId(null);}} className={cn("w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all font-bold text-sm", activeMonth === i ? "bg-blue-600 text-white shadow-lg" : (isDarkMode ? "text-slate-400 hover:text-white hover:bg-white/5" : "text-slate-500 hover:bg-slate-100"))}>
                <span>{m}</span>
                <span className="text-[10px] opacity-60">0,00 €</span>
              </button>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-8 mr-6">
              <div className="text-right">
                <p className="text-[10px] font-black opacity-50 uppercase tracking-widest leading-none">{t.monthlyTotal}</p>
            ...
            <div className="flex gap-2">
              <button onClick={() => setLanguage(l => l === 'en' ? 'de' : 'en')} className="px-3 py-1.5 border border-white/10 rounded-lg font-black text-xs uppercase">{language}</button>
              <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2.5 border border-white/10 rounded-lg">
                {isDarkMode ? <Sun size={18}/> : <Moon size={18}/>}
              </button>
              <button className="p-2.5 border border-white/10 rounded-lg"><Settings size={18}/></button>
            </div>
          </div>
        </header>
          ...
          <div className="flex items-center gap-4 mb-10">
            <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 overflow-x-auto">
              {filteredCompanies.map(c => (
                <button key={c.id} onClick={() => setActiveCompanyId(c.id)} className={cn("px-6 py-2.5 rounded-lg text-xs font-black transition-all whitespace-nowrap", activeCompanyId === c.id ? "bg-blue-600 text-white shadow-md" : "text-slate-400 hover:text-white")}>
                  {c.name}
                </button>
              ))}
            ...
            <div className="relative flex-1 max-w-xs ml-auto">
              ...
              <input className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-sm font-bold text-white outline-none focus:border-blue-500" placeholder={t.searchPlaceholder} />
            </div>
          </div>
          ...
          {activeCompany ? (
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4">
              <div className="p-10 rounded-[2.5rem] bg-white/5 border border-white/10 flex justify-between items-center">
                 ...
                 <button onClick={handleAddHotel} className="px-10 py-5 bg-blue-600 text-white font-black rounded-2xl flex items-center gap-3 shadow-2xl hover:bg-blue-700 transition-all">
      ...
      {activeModal === 'company' && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-[#020617]/90 backdrop-blur-md p-6">
          <div className="w-full max-w-xl bg-[#0F172A] border border-white/10 rounded-[3rem] p-12 shadow-2xl animate-in zoom-in-95">
            <h3 className="text-3xl font-black text-white mb-8 text-center uppercase">{t.sidebar.newCompanyName}</h3>
            <input autoFocus value={newCompanyName} onChange={(e) => setNewCompanyName(e.target.value)} className="w-full rounded-2xl py-6 px-8 font-bold outline-none mb-10 text-xl border border-white/10 bg-white/5 text-white focus:border-blue-500" placeholder="e.g. Siemens" />
            <div className="flex gap-4">
              <button onClick={() => setActiveModal(null)} className="flex-1 py-5 font-black opacity-40 uppercase text-[10px]">{t.common.cancel}</button>
              <button onClick={handleAddCompany} className="flex-1 py-5 bg-blue-600 text-white rounded-2xl font-black shadow-2xl">Create Profile</button>
            </div>
          </div>
        </div>
      )}
