...
export default function DurationCard({
  ...
  return (
    <div className={cn(
      ...
      <div className="flex flex-wrap xl:flex-nowrap items-center justify-between gap-4 p-4">
        
        {/* LEFT GROUP: DATES & ADDERS */}
                ...
                {ROOM_TYPES.map(rt => {
                  ...
                  if (rt === 'WG' && isAddingWg) {
                    return (
                      <div key="wg-input" className={cn('flex items-center h-full rounded-lg border overflow-hidden shrink-0 shadow-sm', dk ? 'border-white/10 bg-[#1E293B]' : 'border-slate-300 bg-white')}>
                        ...
                        <button onClick={() => { handleAddRoomCard('WG', wgBeds); setIsAddingWg(false); }} className="px-4 h-full bg-blue-600 hover:bg-blue-700 text-white font-black text-sm transition-colors border-l border-blue-700">{lang === 'de' ? 'WG Hinzufügen' : 'Add WG'}</button>
                  ...
                  return (
                    <div key={rt} className="flex items-center h-full shadow-sm rounded-lg overflow-hidden border shrink-0" style={{ borderColor: dk ? 'rgba(255,255,255,0.1)' : '#cbd5e1' }}>
                      ...
                      {/* CLEANER ROOM COUNTER */}
                      <button className={cn('px-4 h-full text-sm font-black', dk ? 'bg-teal-500/10 text-teal-400' : 'bg-teal-50 text-teal-700')}>{rt} <span className="opacity-50 mx-1">•</span> {count}</button>
        ...
        {/* RIGHT GROUP: CLEAN INFO DISPLAY & TRASH */}
        {hasDates && (
            <div className="flex items-center gap-5 shrink-0 ml-auto">
              <div className="flex items-center gap-5 text-slate-500 dark:text-slate-400">
                  <span className="flex items-center gap-1.5 text-sm font-bold"><Moon size={16} /> {nights}</span>
                  <span className="flex items-center gap-1.5 text-sm font-bold"><DoorClosed size={16} /> {roomCards.length}</span>
              </div>
              
              <div className={cn("w-px h-6", dk ? "bg-white/10" : "bg-slate-300")}></div>
              
              <div className="flex items-center gap-3">
                  <span className={cn('text-lg font-black flex items-center gap-1.5', dk ? 'text-slate-200' : 'text-slate-800')}><Bed size={18} /> {totalBeds}</span>
                  {/* LOGIC FIX: Only show badge if beds > 0 */}
                  {totalBeds > 0 && (
                      freeBeds > 0 ? (
                          <span className="px-2.5 py-1 rounded bg-red-500/10 text-red-500 text-xs font-black uppercase tracking-wide">{freeBeds} {lang === 'de' ? 'FREI' : 'FREE'}</span>
                      ) : isExpired ? (
                          <span className="px-2.5 py-1 rounded bg-slate-500/10 text-slate-500 text-xs font-black tracking-wide">{lang === 'de' ? 'ABGELAUFEN' : 'EXPIRED'}</span>
                      ) : (
                          <span className="px-2.5 py-1 rounded bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-black tracking-wide">{lang === 'de' ? 'VOLL' : 'FULL'}</span>
                      )
                  )}
              </div>
              ...
              <button onClick={() => setConfirm(true)} className={cn("p-2.5 rounded-xl flex items-center justify-center transition-colors shrink-0", dk ? "text-slate-500 hover:text-red-400 hover:bg-red-500/10" : "text-slate-400 hover:text-red-500 hover:bg-red-50")}>
                <Trash2 size={20} />
              </button>
            </div>
        )}
