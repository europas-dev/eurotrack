...
import { cn, getEmployeeStatus, getStatusColor } from '../lib/utils';
...
export default function EmployeeSlot({
  slotIndex, durationId, employee, durationStart, durationEnd, isDarkMode, onUpdate
}: EmployeeSlotProps) {
  const [editing, setEditing] = useState(false);
  const [saving, setSaving] = useState(false);
  const [name, setName] = useState(employee?.name || '');
  const [checkIn, setCheckIn] = useState(employee?.checkIn || employee?.check_in || durationStart);
  const [checkOut, setCheckOut] = useState(employee?.checkOut || employee?.check_out || durationEnd);
  const dk = isDarkMode;

  const status = employee
    ? getEmployeeStatus(employee.checkIn || employee.check_in || '', employee.checkOut || employee.check_out || '')
    : null;

  const statusColors: Record<string, string> = {
    active: 'text-green-400 border-green-500/30 bg-green-500/10',
    'ending-soon': 'text-amber-400 border-amber-500/30 bg-amber-500/10',
  ...
  const handleRemove = async () => {
    if (!employee?.id) { onUpdate(slotIndex, null); return; }
    try {
      setSaving(true);
      await deleteEmployee(employee.id);
      onUpdate(slotIndex, null);
      setName(''); setCheckIn(durationStart); setCheckOut(durationEnd);
      setEditing(false);
    } catch(e) { console.error(e); }
    finally { setSaving(false); }
  };
  ...
  if (!employee && !editing) {
    return (
      <button
        onClick={() => { setEditing(true); setName(''); setCheckIn(durationStart); setCheckOut(durationEnd); }}
        className={cn(
          "w-full py-3 px-3 rounded-xl border-2 border-dashed text-xs font-bold flex items-center gap-2 transition-all",
          dk ? "border-white/10 text-slate-600 hover:border-blue-500/40 hover:text-blue-400"
      ...
      <div className={cn("p-3 rounded-xl border space-y-2",
        ...
        <div className="flex gap-2">
          <div className="flex-1">
            <label className={cn("text-[9px] font-bold uppercase tracking-widest block mb-1",
              dk ? "text-slate-500" : "text-slate-400")}>Check-in</label>
            <input type="date" value={checkIn} min={durationStart} max={durationEnd}
              onChange={e => setCheckIn(e.target.value)} className={inputCls} />
          </div>
          <div className="flex-1">
            <label className={cn("text-[9px] font-bold uppercase tracking-widest block mb-1",
              dk ? "text-slate-500" : "text-slate-400")}>Check-out</label>
            <input type="date" value={checkOut} min={durationStart} max={durationEnd}
  ...
  return (
    <div
      onClick={() => { setName(employee.name); setCheckIn(employee.checkIn || employee.check_in || durationStart); setCheckOut(employee.checkOut || employee.check_out || durationEnd); setEditing(true); }}
      className={cn(
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          <div className={cn("w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black flex-shrink-0",
            dk ? "bg-white/10 text-white" : "bg-slate-100 text-slate-700")}>
            {employee.name?.charAt(0)?.toUpperCase() || '?'}
          </div>
          <div className="min-w-0">
            <p className={cn("text-xs font-bold truncate", dk ? "text-white" : "text-slate-900")}>{employee.name}</p>
            <p className={cn("text-[9px] truncate", dk ? "text-slate-500" : "text-slate-400")}>
              {employee.checkIn || employee.check_in} → {employee.checkOut || employee.check_out}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          {status && (
            <span className={cn("text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-full border", statusColors[status])}>
              {status}
            </span>
          )}
          {saving
