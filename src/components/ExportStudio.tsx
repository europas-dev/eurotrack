    ...
    <div className="fixed inset-0 z-[9999] bg-slate-900/90 backdrop-blur-md flex flex-col animate-in fade-in duration-300">
      ...
      <div className="h-16 border-b border-white/10 flex items-center justify-between px-8 bg-slate-900 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => generatePDF(reportData, activeCols, title, lang, total, false)} className="flex items-center gap-2 px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white rounded-lg font-bold text-sm transition-all"><FileText size={16}/> PDF</button>
          <button onClick={() => generatePDF(reportData, activeCols, title, lang, total, true)} className="flex items-center gap-2 px-4 py-2 bg-white text-slate-900 hover:bg-slate-100 rounded-lg font-bold text-sm transition-all"><Printer size={16}/> {isDe ? 'Drucken' : 'Print'}</button>
        </div>
      </div>
