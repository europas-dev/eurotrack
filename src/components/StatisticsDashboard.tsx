  ...
  const stats = useMemo(() => {
    ...
    (hotels || []).forEach(h => {
      ...
      (h.invoices || []).forEach((inv: any) => {
        ...
        // EXACT ALIGNMENT WITH DASHBOARD.TSX ROUNDING
        const roundedInvBrutto = Math.round(invBrutto * 100) / 100;

        if (inv.isPaid) {
          rawPaid += roundedInvBrutto;
        } else {
          rawUnpaid += roundedInvBrutto;
          if (inv.dueDate && new Date(inv.dueDate) < today) {
            rawOverdue += roundedInvBrutto;
          }
        }
      });
    ...
    // Clean up floating points for charts
    months.forEach(m => {
