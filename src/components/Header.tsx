...
export default function Header({
  ...
  // FIXED: iconBtn is now correctly defined
  ...
  const CollabRow = ({ c }: { c: any }) => (
    <div title={`${c.fullName || ''}\n${c.email}`} className={cn('flex items-center gap-3 px-3 py-2.5 rounded-xl border', dk ? 'bg-white/3 border-white/8' : 'bg-slate-50 border-slate-200')}>
      ...
      <select 
        ...
        className={cn('text-xs font-bold rounded-lg border px-2 py-1 outline-none transition-all', dk ? 'bg-[#1E293B] border-white/10 text-white' : 'bg-white border-slate-200 text-slate-900')}
      >
        <option value="viewer">👁 Viewer</option>
        <option value="editor">✏️ Editor</option>
        <option value="admin">🛡️ Admin</option>
      </select>
      <button onClick={e => { e.stopPropagation(); handleChangeCollabRole(c.userId); }} disabled={changingRole === c.userId} className="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-bold rounded-lg transition-all">
                    ...
                    <>
                      ...
                      <div className={cn('rounded-xl border p-4 space-y-4', dk ? 'border-white/10' : 'border-slate-200')}>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            <p className={cn('text-xs font-bold mb-1.5', dk ? 'text-slate-400' : 'text-slate-600')}>{lang === 'de' ? 'Appearance' : 'Appearance'}</p>
                            <div className="flex rounded-lg border overflow-hidden">
                              <button onClick={toggleTheme} className={cn('flex-1 py-1.5 flex justify-center', dk ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-400')}><Moon size={14} /></button>
                              <button onClick={toggleTheme} className={cn('flex-1 py-1.5 flex justify-center', !dk ? 'bg-blue-600 text-white' : 'bg-white/5 text-slate-400')}><Sun size={14} /></button>
                            </div>
                          </div>
                        </div>
