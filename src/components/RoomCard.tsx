    ...
    return (
      <div className={cn('flex items-center justify-between px-4 py-2 rounded-lg border', dk ? 'bg-red-900/20 border-red-500/30' : 'bg-red-50 border-red-200')}>
        ...
        <div className="flex gap-2">
          <button onClick={remove} disabled={saving || viewOnly} className="px-4 py-1.5 rounded bg-red-600 text-white text-xs font-bold">{lang === 'de' ? 'Ja' : 'Yes'}</button>
...
function InlineNMBRow({
  ...
  card, dk, lang, onPatch, disabled, multiplier, activeTab, queueSave, viewOnly // ADDED viewOnly
}: {
  ...
  return (
    <div className={cn("flex items-start gap-4 flex-wrap", disabled && "opacity-50 pointer-events-none")}>
        ...
        <div className="flex items-start gap-1 flex-nowrap">
          ...
          {currentDiscountValue == null ? (
            <div className="mt-[22px] shrink-0">
              {/* SURGICAL FIX: Hide discount add button for Viewers */}
              {!viewOnly && (
            ...
            <div className="flex flex-col shrink-0">
              ...
              <div className="flex items-center gap-1.5">
                <div className="relative flex items-center h-[38px] w-[95px]">
                  {/* SURGICAL FIX: Disable discount input if viewOnly */}
                  <input 
                    ...
                    className={cn((disabled || viewOnly) ? disabledInputCls : inputClsBase, 'w-full pr-8 h-full text-sm font-black')} 
                    placeholder="0" 
                  />
                  {/* SURGICAL FIX: Disable discount toggle if viewOnly */}
                  <button 
                    disabled={disabled || viewOnly}
                    onClick={() => queueSave({ [discountTypeKey]: currentDiscountType === 'percentage' ? 'fixed' : 'percentage'} as any)} 
                    className={cn("absolute right-1 w-7 h-7 rounded flex items-center justify-center text-[12px] font-black border transition-colors", dk ? "bg-white/10 border-white/10 text-white hover:bg-white/20" : "bg-slate-100 border-slate-300 text-slate-700 hover:bg-slate-200", (disabled || viewOnly) && "cursor-default opacity-60")}
                  >
                    {currentDiscountType === 'percentage' ? '%' : '€'}
                  </button>
                </div>
                {/* SURGICAL FIX: Hide discount clear button for Viewers */}
                {!viewOnly && (
      ...
      <div className="flex flex-col shrink-0">
        ...
        <MwstInput value={card[mwstKey] as any} onChange={(v) => updateMwst(v || '')} isDarkMode={dk} disabled={disabled || viewOnly} />
        <div className={sumLbl}/>
      </div>

      <div className="flex flex-col shrink-0">
        <p className={lbl}>Brutto (€)</p>
        <input type="number" min={0} step="0.01" value={bVal} onChange={e => updateBrutto(e.target.value)} disabled={disabled || viewOnly} style={noSpinner} className={cn((disabled || viewOnly) ? disabledInputCls : inputClsBase, 'w-24 h-[38px]', card[nettoKey] != null && (dk ? 'bg-white/5 text-slate-400' : 'bg-slate-100 text-slate-400'))} placeholder="Auto" />
        <div className={cn(sumLbl, "min-h-[16px]")}>
      ...
      {energyNettoKey && (
        <div className={cn("flex items-start gap-4 px-4 py-2.5 rounded-xl border border-dashed ml-2 shrink-0", dk ? "border-yellow-500/30 bg-yellow-500/5" : "border-yellow-400/60 bg-yellow-50/50")}>
          ...
          <div className="flex flex-col shrink-0">
            <p className={lbl}>MwSt</p>
            <MwstInput value={card[energyMwstKey!] as any} onChange={(v) => updateEnergyMwst(v || '')} isDarkMode={dk} disabled={disabled || viewOnly} />
            <div className={sumLbl}/>
          </div>
          <div className="flex flex-col shrink-0">
        ...
        {!isOpen ? (
           <>
             ...
             <div className="flex flex-col items-end shrink-0 ml-4">
                {isMasterPricingActive ? <span className={cn("text-[10px] font-black uppercase tracking-widest", dk ? "text-slate-600" : "text-slate-400")}>{lang === 'de' ? 'Master Aktiv' : 'Master Active'}</span> : 
                
                {/* --- [LAUNCH TEST: SMART PRICE SYNC BADGE] --- */}
                 {activeTab === 'total_room' && nightsDiff !== 0 && !isMasterPricingActive && (
           ...
           <div className="flex items-center gap-3 flex-1 flex-wrap sm:flex-nowrap">
             ...
             <span className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-blue-500/10 text-blue-500 font-black text-sm shrink-0 ml-1">
             ...
             {!isMasterPricingActive && (
               <div className="flex items-center shrink-0">
                 {/* SURGICAL FIX: Allow viewing pricing view, but stop DB queueSave when toggling tab if viewOnly */}
                 <button onClick={(e) => { 
                    e.stopPropagation(); 
                    setShowPricing(!showPricing);
                 }} className={tabBtn(showPricing)}>{lang === 'de' ? 'Preis' : 'Price'}</button>
                 <div className="flex flex-col items-end min-w-[100px] ml-3"><span className="text-xl font-black">{roomTotalDisplay}</span></div>
               </div>
             )}
             {/* SURGICAL FIX: Hide delete button on open row for viewers */}
             {!viewOnly && (
