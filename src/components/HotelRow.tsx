...
import { Check, ChevronDown, ChevronRight, Clock, Loader2, Plus, Trash2, X, MapPin, User, Phone, Globe, Mail, Building, Star, Search } from 'lucide-react';
import {
...
export function HotelRow({ entry, index, isDarkMode: dk, lang = 'de', searchQuery = '', isPinned = false, onTogglePin = () => {}, companyOptions = [], cityOptions = [], onDelete, onUpdate }: HotelRowProps) {
  ...
  const inputCls = cn('w-full px-3 py-1.5 rounded-lg text-xs font-bold outline-none border transition-all focus:border-blue-500', dk ? 'bg-[#1E293B] border-white/10 text-white placeholder-slate-600' : 'bg-white border-slate-200 text-slate-900 placeholder-slate-400');
  ...
  return (
    <div className="space-y-1 relative" style={{ zIndex: 100 - index }}>
      <div className={cn('rounded-2xl border overflow-hidden transition-all duration-200 shadow-sm', dk ? 'bg-[#0F172A] border-white/5 hover:border-white/10' : 'bg-white border-slate-200 hover:border-slate-300')}>
        ...
        {open && (
          <div className={cn('border-t p-4', dk ? 'bg-[#0B1224] border-white/5' : 'bg-slate-50/50 border-slate-100')} onClick={e => e.stopPropagation()}>
            
            {/* The Perfect 1-Line Flex Grid */}
            <div className="flex flex-wrap lg:flex-nowrap gap-3 items-end">
              ...
              <div className="flex-[2.5_2.5_0%] min-w-[200px]">
                 ...
                 <input value={localHotel.address || ''} onChange={e => patchHotel({ address: e.target.value })} onKeyDown={handleEnterBlur} placeholder={lang === 'de' ? 'Adresse...' : 'Address...'} className={inputCls} />
              </div>
              
              <div className="flex-[1.5_1.5_0%] min-w-[120px]">
                 <label className={labelCls}><User size={10}/> {lang === 'de' ? 'Ansprechpartner' : 'Contact Person'}</label>
                 <input value={localHotel.contactPerson || ''} onChange={e => patchHotel({ contactPerson: e.target.value })} onKeyDown={handleEnterBlur} placeholder={lang === 'de' ? 'Name...' : 'Name...'} className={inputCls} />
              </div>
              ...
              <div className="flex-[1.5_1.5_0%] min-w-[120px]">
                 ...
                 <div className={cn('flex items-center rounded-lg border overflow-hidden transition-all focus-within:border-blue-500', dk ? 'bg-[#1E293B] border-white/10' : 'bg-white border-slate-200')}>
                    <input value={localHotel.phone || ''} onChange={e => patchHotel({ phone: e.target.value })} onKeyDown={handleEnterBlur} placeholder={lang === 'de' ? 'Telefonnummer...' : 'Phone number...'} className={cn('w-full px-3 py-1.5 text-xs font-bold outline-none bg-transparent', dk ? 'text-white' : 'text-slate-900')} />
                 </div>
              </div>
              ...
              <div className="flex-[1.5_1.5_0%] min-w-[140px]">
                 ...
                 <div className="relative flex items-center">
                   <input value={localHotel.email || ''} onChange={e => patchHotel({ email: e.target.value })} onKeyDown={handleEnterBlur} placeholder="hotel@example.com" className={cn(inputCls, 'pr-8')} />
                   {localHotel.email && (
                     <a href={`mailto:${localHotel.email}`} className="absolute right-1.5 p-1 bg-blue-500 text-white rounded hover:bg-blue-600 transition-all shadow-sm">
                       <Mail size={10} />
                     </a>
                   )}
                 </div>
              ...
              <div className="flex-[1.5_1.5_0%] min-w-[140px]">
                 ...
                 <div className="relative flex items-center">
                   <input value={localHotel.website || ''} onChange={e => patchHotel({ website: e.target.value })} onKeyDown={handleEnterBlur} placeholder="www.hotel.com" className={cn(inputCls, 'pr-8')} />
                   {localHotel.website && (
                     <a href={localHotel.website.startsWith('http') ? localHotel.website : `https://${localHotel.website}`} target="_blank" rel="noreferrer" className="absolute right-1.5 p-1 bg-emerald-500 text-white rounded hover:bg-emerald-600 transition-all shadow-sm">
                       <Globe size={10} />
                     </a>
                   )}
                 </div>
            ...
            <textarea className={cn(inputCls, 'min-h-[50px] resize-y font-normal mt-4')} value={localHotel.notes || ''} onChange={e => patchHotel({ notes: e.target.value })} placeholder={lang === 'de' ? 'Notizen...' : 'Notes...'} />
            ...
            {(localHotel.durations || []).length > 0 ? (
              ...
              />
               ...
               <button onClick={addDuration} disabled={creatingDuration}
                className={cn('w-full mt-2 py-4 rounded-xl border-2 border-dashed text-sm font-bold transition-all',
...
export function ModernDropdown({ value, options, onChange, isDarkMode, lang, placeholder = 'Select' }: { value: string, options: string[], onChange: (v: string) => void, isDarkMode: boolean, lang?: string, placeholder?: string }) {
  ...
  return (
    <div ref={ref} className="relative w-full">
      <button onClick={() => setOpen(!open)} className={cn('w-full px-3 py-1.5 flex items-center justify-between rounded-lg border text-xs font-bold outline-none transition-all', isDarkMode ? 'bg-[#1E293B] border-white/10 text-white hover:border-blue-500' : 'bg-white border-slate-200 text-slate-900 hover:border-blue-500')}>
      ...
      {open && (
        <div className={cn('absolute top-full mt-1 left-0 right-0 z-50 rounded-xl border shadow-xl py-1 overflow-hidden', isDarkMode ? 'bg-[#0F172A] border-white/10' : 'bg-white border-slate-200')}>
          <div className="max-h-48 overflow-y-auto no-scrollbar">
            {allOptions.map(opt => (
              <button key={opt} onClick={() => { onChange(opt); setOpen(false); }} className={cn('w-full text-left px-3 py-1.5 text-xs font-bold transition-all', value === opt ? (isDarkMode ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-50 text-blue-600') : (isDarkMode ? 'text-slate-300 hover:bg-white/5' : 'text-slate-700 hover:bg-slate-50'))}>
                {opt}
              </button>
            ))}
          </div>
          ...
          {!addingNew ? (
            <button onClick={() => { setAddingNew(true); setNewVal(''); }} className={cn('w-full text-left px-3 py-1.5 text-xs font-bold flex items-center gap-1.5 transition-all', isDarkMode ? 'text-blue-400 hover:bg-white/5' : 'text-blue-600 hover:bg-blue-50')}>
  ...
  if (!editing) {
    return (
      <span onClick={e => { e.stopPropagation(); setDraft(value); setEditing(true); }} className="cursor-text hover:underline inline-block min-w-[30px]">
        {value || <span className={isDarkMode ? 'text-slate-600' : 'text-slate-300'}>{lang === 'de' ? 'Stadt...' : 'City...'}</span>}
      </span>
    );
  }
  return (
      ...
      <div className="flex flex-wrap gap-1 cursor-pointer min-h-[22px]" onClick={() => setOpen(o => !o)}>
        {selected.length > 0 ? selected.map(tag => (
          <span key={tag} className={cn('inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-[10px] font-bold border', dk ? 'bg-white/5 border-white/10 text-slate-300' : 'bg-slate-50 border-slate-200 text-slate-700')}>
            {tag} <span onMouseDown={e => { e.preventDefault(); removeTag(tag, e); }} className="cursor-pointer hover:opacity-70 ml-0.5"><X size={8} /></span>
          </span>
        )) : (
