...
// --- SEAMLESS AUTOCOMPLETE INPUT (VIEW-ONLY READY) ---
function SeamlessInput({ value, options, isDarkMode, onChange, placeholder, className, textClass, searchQuery, disabled }: any) {
  ...
  if (!editing || disabled) {
    return (
      <div className={cn("truncate transition-opacity w-full min-h-[20px]", !disabled ? "cursor-text hover:opacity-70" : "cursor-default", textClass)} 
           onClick={(e) => { if (disabled) return; e.stopPropagation(); setEditing(true); setDraft(value || ''); setShowOptions(true); }}>
...
// --- MINI DROPDOWN COMPONENT FOR MWST (VIEW-ONLY READY) ---
function MwstInput({ value, onChange, isDarkMode, disabled }: { value: string | null, onChange: (v: string | null) => void, isDarkMode: boolean, disabled?: boolean }) {
  ...
  return (
    <div ref={ref} className="relative flex items-center h-[34px]">
      ...
      <button disabled={disabled} onClick={() => setOpen(!open)} className={cn('px-1.5 h-full rounded-r-lg border border-l-0 transition-all flex items-center justify-center', isDarkMode ? 'bg-white/5 border-white/10 text-slate-400 hover:bg-white/10' : 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100', disabled && "hidden")}><ChevronDown size={14} /></button>
      {open && (
...
export function HotelRow({ entry, index, isDarkMode: dk, lang = 'de', searchQuery = '', selectedMonth = null, selectedYear = null, companyOptions = [], cityOptions = [], hotelOptions = [], onDelete, onUpdate, onDeleteCompanyOption, onAddOption, viewOnly }: any) {
  ...
  // --- THE MASTER ACCOUNTING ENGINE REMAINED UNCHANGED ---
  const masterMath = useMemo(() => {
    ...
    const baseCostsWithDisplay = (localHotel.baseCosts || []).map((bc: any) => {
        ...
        if (bc.netto != null) {
            ...
            if (isMwstValid) { bBrutto = discountedNetto * (1 + bMwSt/100); bBruttoDisplay = bBrutto.toFixed(2); }
    ...
    const extrasWithDisplay = (localHotel.extraCosts || []).map((ec: any) => {
       ...
       if (ec.netto != null) {
         eNetto = parseFloat(ec.netto); eNettoDisplay = ec.netto.toString();
         if (eMwstValid) { eBrutto = eNetto * (1 + eMwst!/100); eBruttoDisplay = eBrutto.toFixed(2); }
       } else if (ec.brutto != null) {
         eBrutto = parseFloat(ec.brutto); eBruttoDisplay = ec.brutto.toString();
         if (eMwstValid) { eNetto = eBrutto / (1 + eMwst!/100); eNettoDisplay = eNetto.toFixed(2); }
       }
       extraNettoTotal += eNetto; extraBruttoTotal += eBrutto;
    ...
    if (localHotel.has_global_discount && localHotel.global_discount_value) {
       ...
       if (target === 'netto') {
          ...
          Object.keys(buckets).forEach(k => { buckets[k] = buckets[k] * (1 - ratio); finalBrutto += buckets[k]; });
       } else finalBrutto = Math.max(0, preGlobalBrutto - (isFixed ? gVal : preGlobalBrutto * (gVal/100)));
    }

    const displayBrutto = localHotel.override_total_brutto != null ? parseFloat(localHotel.override_total_brutto) : finalBrutto;
    let pricePerBed = 0;
    if (localHotel.override_price_per_bed != null) pricePerBed = parseFloat(localHotel.override_price_per_bed);
    else if (!isMasterActive && minPricePerBed !== null) pricePerBed = minPricePerBed;

    return { freeBeds: tFree, totalBeds: tBeds, employees: allEmps, displayNetto: finalNetto, displayBrutto, buckets, pricePerBed, baseCostsWithDisplay, extrasWithDisplay, isMasterActive, isOverriddenBrutto: localHotel.override_total_brutto != null, isOverriddenBed: localHotel.override_price_per_bed != null };
  }, [localHotel]);
  ...
  function patchHotel(changes: any) {
    if (viewOnly) return; // CRITICAL UI LOCK
          ...
          <div className="flex-[2.5] px-2">
            <div className="flex flex-wrap gap-1.5">
              {visibleEmps.map((emp: any, i: number) => {
                ...
                return (
                  <button key={emp.id || i} title={tooltip} onClick={(e) => { e.stopPropagation(); setOpen(true); const durationIdx = localHotel.durations.findIndex((d: any) => d.id === emp.duration_id || d.id === emp.durationId); if (durationIdx !== -1) setActiveDurationTab(durationIdx); setTimeout(() => { const element = document.getElementById(`emp-slot-${emp.id}`); if (element) { element.scrollIntoView({ behavior: 'smooth', block: 'center' }); element.classList.add('ring-2', 'ring-teal-500'); setTimeout(() => element.classList.remove('ring-2', 'ring-teal-500'), 2000); } }, 100); }} 
          ...
          <div className={cn('p-6 space-y-6 rounded-b-2xl border-t', dk ? 'bg-[#0B1224] border-white/5' : 'bg-slate-50 border-slate-200')} onClick={e => e.stopPropagation()}>
                           ...
                           {masterMath.baseCostsWithDisplay.map((bc: any, index: number) => (
                              <div key={bc.id} style={{ zIndex: 60 - index, position: 'relative' }} className={cn("flex flex-col md:flex-row md:items-start justify-between gap-4 p-3 rounded-xl border", dk ? "bg-white/5 border-white/5" : "bg-slate-50 border-slate-200")}>
                                 ...
                                 <div className="flex-1 flex flex-wrap xl:flex-nowrap items-start gap-2 justify-end w-full pt-1">
                                    ...
                                    <div className="flex items-center gap-1.5 shrink-0"><span className={labelCls}>MwSt</span><MwstInput disabled={viewOnly} value={bc.mwst} onChange={(v) => updateBaseCost(bc.id, {mwst: v})} isDarkMode={dk} /></div>
                                    ...
                                    {localHotel.baseCosts.length > 1 && !viewOnly ? (
                           ...
                           <div className="flex-1 flex flex-col gap-2 items-end w-full">
                               {masterMath.extrasWithDisplay.map((ec:any, index: number) => (
                                  <div key={ec.id} style={{ zIndex: 40 - index, position: 'relative' }} className="flex flex-nowrap items-center gap-2 w-full justify-end">
                                     ...
                                     <div className="flex items-center gap-1.5 shrink-0"><span className={labelCls}>MwSt</span><MwstInput disabled={viewOnly} value={ec.mwst} onChange={(v) => updateExtra(ec.id, {mwst: v})} isDarkMode={dk} /></div>
                   ...
                   <div className={cn("pt-4 border-t flex flex-col gap-3 mt-auto", dk ? "border-white/10" : "border-slate-200")}>
                      <div className="flex justify-between items-center group w-full">
                         ...
                         {editingOBrutto && !viewOnly ? (
                           ...
                           <span onClick={() => {!viewOnly && setEditBruttoValue(masterMath.displayBrutto.toFixed(2)); !viewOnly && setEditingOBrutto(true);}} className={cn("text-[22px] font-black rounded px-1 -mr-1 transition-colors flex items-center gap-2 text-right ml-auto", !viewOnly ? "cursor-pointer" : "cursor-default", masterMath.isOverriddenBrutto ? "text-yellow-500 bg-yellow-500/10" : dk ? "text-white group-hover:bg-white/10" : "text-slate-900 group-hover:bg-slate-200")}>{formatCurrency(masterMath.displayBrutto)} {!viewOnly && <Edit3 size={14} className="opacity-0 group-hover:opacity-100"/>}</span>
                         )}
                      </div>
                      
                      <div className="flex justify-between items-center group w-full">
                        <span className={cn("text-[11px] font-bold shrink-0", dk ? "text-slate-500" : "text-slate-400")}>{lang === 'de' ? 'Preis / Bett' : 'Price / Bed'}</span>
                        {editingPriceBed && !viewOnly ? (
                           ...
                           <span onClick={() => {!viewOnly && setEditPriceBedValue(masterMath.pricePerBed.toFixed(2)); !viewOnly && setEditingPriceBed(true);}} className={cn("text-[14px] font-bold rounded px-1 -mr-1 transition-colors flex items-center gap-1.5 text-right ml-auto", !viewOnly ? "cursor-pointer" : "cursor-default", masterMath.isOverriddenBed ? "text-yellow-600 bg-yellow-500/10" : dk ? "text-slate-300 group-hover:bg-white/10" : "text-slate-600 group-hover:bg-slate-200")}>
                             {!masterMath.isOverriddenBed && masterMath.pricePerBed > 0 ? 'ab ' : ''}{formatCurrency(masterMath.pricePerBed)} / N {!viewOnly && <Edit3 size={11} className="opacity-0 group-hover:opacity-100"/>}
                           </span>
                        )}
                      </div>
                   </div>
            ...
            <div className="pt-4">
              <div className="flex items-end gap-1 flex-wrap">
                ...
                {!viewOnly && (
                  <button onClick={async () => { setCreatingDuration(true); const created = await createDuration({ hotelId: localHotel.id }); const next = { ...localHotel, durations: [...localHotel.durations, { ...created, roomCards: [] }] }; setLocalHotel(next); onUpdate(localHotel.id, next); setActiveDurationTab(next.durations.length - 1); setCreatingDuration(false); }} 
                    className={cn("px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all border border-dashed mb-[3px] ml-1", dk ? "border-white/20 text-slate-400 hover:bg-white/10 hover:text-white" : "border-slate-300 text-slate-500 hover:bg-slate-200 hover:text-slate-800")}>
            ...
            {localHotel.durations[activeDurationTab] ? (
              <div className={cn("relative z-0", activeDurationTab === 0 ? "[&>div]:rounded-tl-none" : "")}>
                <DurationCard duration={localHotel.durations[activeDurationTab]} isDarkMode={dk} lang={lang} 
                  isMasterPricingActive={masterMath.isMasterActive} viewOnly={viewOnly}
                  onUpdate={(id, upd) => { const next = { ...localHotel, durations: localHotel.durations.map((d: any) => d.id === id ? upd : d) }; setLocalHotel(next); onUpdate(localHotel.id, next); }}
                  onDelete={(id) => { const next = { ...localHotel, durations: localHotel.durations.filter((d: any) => d.id !== id) }; setLocalHotel(next); onUpdate(localHotel.id, next); enqueue({ type: 'deleteDuration', payload: { id } }); if (activeDurationTab >= next.durations.length) setActiveDurationTab(Math.max(0, next.durations.length - 1)); }}
                />
              </div>
            ) : null}
          </div>
