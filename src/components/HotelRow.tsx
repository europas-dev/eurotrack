...
function InlineEdit({
  ...
  return (
    <button
      ...
      className={cn(
        'w-full text-left px-1 py-0.5 rounded transition-all truncate block',
        textClass,
      ...
      <div className={cn(
        ...
        <div className="px-4 py-2.5">
          <div className="flex items-center gap-2.5 w-full">
            ...
            <div className="w-[190px] flex-shrink-0 min-w-0">
              <InlineEdit value={localHotel.name || ''} onChange={v => patchHotel({ name: v })}
            ...
            <div className="w-[100px] flex-shrink-0 min-w-0">
              <InlineEdit value={localHotel.city || ''} onChange={v => patchHotel({ city: v })}
            ...
            <div className="w-[110px] flex-shrink-0 min-w-0">
              <InlineEdit value={localHotel.companyTag || ''} onChange={v => patchHotel({ companyTag: v })}
            ...
            {/* Duration tags — flex-1 */}
            <div className="flex-1 min-w-0 flex items-center gap-1 overflow-hidden">
              {(localHotel.durations ?? []).slice(0, 3).map((d: any, i: number) => (
                <span key={d.id || i} className={cn(
                  'px-2 py-0.5 rounded text-[10px] font-bold whitespace-nowrap flex-shrink-0 border',
                  dk ? 'bg-white/5 border-white/10 text-slate-300' : 'bg-slate-100 border-slate-200 text-slate-600'
                )}>{durTag(d)}</span>
              ))}
              {(localHotel.durations ?? []).length > 3 && (
                <span className={cn('text-[10px] font-bold', dk ? 'text-slate-500' : 'text-slate-400')}>
                  +{localHotel.durations.length - 3}
                </span>
              )}
            ...
            {/* Employee pills — 160px max */}
            <div className="w-[160px] flex-shrink-0 flex items-center gap-1 overflow-hidden">
              {allEmployees.slice(0, 4).map((emp: any, i: number) => (
                <div key={emp.id || i}
                  className={cn('px-2 py-0.5 rounded-full border text-[10px] font-bold whitespace-nowrap flex-shrink-0', pillCls(emp))}
        ...
        {open && (
          <div className={cn('border-t px-4 py-4 space-y-4',
            ...
            {/* Info row — 5 fields in one line */}
            <div className="flex gap-3 flex-wrap">
              {[
                { field: 'address',       de: 'Adresse',         en: 'Address' },
                { field: 'contactPerson', de: 'Ansprechpartner', en: 'Contact person' },
                { field: 'phone',         de: 'Telefon',         en: 'Phone' },
                { field: 'email',         de: 'E-Mail',          en: 'Email' },
                { field: 'webLink',       de: 'Webseite',        en: 'Website' },
              ].map(({ field, de, en }) => (
                <div key={field} className="flex flex-col gap-0.5 min-w-[120px] flex-1">
                  <span className={cn('text-[10px] font-bold uppercase tracking-widest', dk ? 'text-slate-500' : 'text-slate-400')}>
                    {lang === 'de' ? de : en}
                  </span>
                  <input type="text" value={(localHotel[field] as string) || ''} placeholder="–"
                    onChange={e => patchHotel({ [field]: e.target.value })} className={inputCls} />
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <textarea value={localHotel.notes || ''} placeholder={lang === 'de' ? 'Interne Notizen...' : 'Internal notes...'}
                onChange={e => patchHotel({ notes: e.target.value })} className={cn(inputCls, 'min-h-[80px] resize-y')} />
            </div>
