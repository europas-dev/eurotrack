...
export default function HotelRow({
  ...
  function patchHotel(changes: any) {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        ...
        await offlineSync.enqueue({ type: 'updateHotel', payload: { id: localHotel.id, ...next } });
        onUpdate(localHotel.id, next);
      } finally {
        setSaving(false);
      }
  ...
  async function handleDelete() {
    try { await deleteHotel(localHotel.id); }
    catch { await offlineSync.enqueue({ type: 'deleteHotel', payload: { id: localHotel.id } }); }
  ...
  async function addDuration() {
    try {
      ...
      const dur = { ...created, employees: [] };
      const nextDurations = [...(localHotel.durations || []), dur];
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1 overflow-x-auto">
              <div className="min-w-[180px] flex-shrink-0">
                <p className={cn('text-sm font-black leading-tight', dk?'text-white':'text-slate-900')}>{localHotel.name}</p>
                <p className={cn('text-[11px] uppercase tracking-widest leading-tight', dk?'text-slate-500':'text-slate-400')}>{localHotel.city}</p>
              </div>
              ...
              <div className="text-center min-w-[72px] flex-shrink-0">
                <p className={cn('text-sm font-black', freeBeds>0?'text-amber-400':'text-green-400')}>{freeBeds}</p>
              ...
              <div className={cn('text-sm font-black min-w-[100px] text-right flex-shrink-0', dk?'text-white':'text-slate-900')}>
        ...
        {open && (
          <div className={cn('border-t p-4 space-y-4', dk?'border-white/10 bg-white/[0.02]':'border-slate-200 bg-slate-50/50')}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <input className={inputCls} value={localHotel.webLink||''} onChange={e=>patchHotel({webLink:e.target.value})} placeholder={lang==='de'?'Webseite...':'Website...'}/>
              <input className={inputCls} value={localHotel.contactPerson||''} onChange={e=>patchHotel({contactPerson:e.target.value})} placeholder={lang==='de'?'Ansprechpartner...':'Contact person...'}/>
            </div>

            <textarea className={cn(inputCls,'min-h-[90px] resize-y')} value={localHotel.notes||''} onChange={e=>patchHotel({notes:e.target.value})} placeholder={lang==='de'?'Notizen...':'Notes...'}/>
