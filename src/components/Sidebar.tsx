  ...
  return (
    <aside className={cn(
      ...
      <div className={cn('p-4 border-b flex items-center justify-between', dk ? 'border-white/5' : 'border-slate-200')}>
        {!collapsed && (
          <span className={cn('text-sm font-bold uppercase tracking-widest', dk ? 'text-white' : 'text-slate-900')}>
            EuroTrack
          </span>
        )}
        <button onClick={onToggleCollapse}
          className={cn('p-1.5 rounded-lg transition-all', dk ? 'hover:bg-white/10 text-slate-400' : 'hover:bg-slate-100 text-slate-600')}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        {!collapsed && (
          <div>
            <label className={cn('text-xs font-bold uppercase tracking-widest mb-2 block', dk ? 'text-slate-400' : 'text-slate-600')}>
              {lang === 'de' ? 'Jahr' : 'Year'}
            </label>
            <select value={selectedYear} onChange={e => setSelectedYear(parseInt(e.target.value))}
              className={cn('w-full px-3 py-2 rounded-lg border text-sm font-bold outline-none',
      ...
      {/* Yearly total */}
      {!collapsed && (
        <div className={cn('p-4 border-t', dk ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200')}>
          <p className={cn('text-xs font-bold uppercase tracking-widest mb-1', dk ? 'text-slate-400' : 'text-slate-600')}>
