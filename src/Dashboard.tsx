...
export default function Dashboard({ theme, lang, toggleTheme, setLang, viewOnly = false, accessLevel, onSignOut, offlineMode, onToggleOfflineMode }: DashboardProps) {
  ...
  // --- DATA FETCHING LOGIC WITH MASTER FIELDS ---
  useEffect(() => { 
    let isMounted = true;
    setLoading(true);
    setError(''); 
  ...
  const activeFilters = useMemo(() => {
    ...
    if (filterPaid !== 'all') {
      badges.push({ 
        id: 'paid', 
        label: lang === 'de' ? 'Zahlung' : 'Payment', 
        val: lang === 'de' ? (filterPaid === 'paid' ? 'Bezahlt' : 'Offen') : filterPaid, 
        clear: () => setFilterPaid('all') 
      });
    }
    ...
    if (groupBy !== 'none') {
      badges.push({ 
        ...
        val: lang === 'de' && groupBy === 'company' ? 'Firma' : groupBy, 
        clear: () => setGroupBy('none') 
      });
    }
    if (sortBy !== 'created_at' || sortDir !== 'desc') {
        ...
        badges.push({ 
          ...
          val: `${label} (${sortDir === 'asc' ? (lang === 'de' ? 'AUF' : 'ASC') : (lang === 'de' ? 'AB' : 'DESC')})`, 
