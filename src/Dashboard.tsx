...
export default function Dashboard({ 
  ...
  // FIXED: Popup overflow and positioning
  const popupCls = cn(
  ...
  useEffect(() => {
    ...
    channel.on('presence', { event: 'sync' }, () => {
      if (!isMounted) return;
      const uniqueUsers = Array.from(new Map(Object.values(channel.presenceState()).flat().map((p: any) => [p.user.id, p.user])).values());
      const filteredUsers = uniqueUsers.filter((u: any) => !(u.id === accessLevel?.id && accessLevel?.role === 'superadmin' && accessLevel?.invisible));
      setActiveUsers(filteredUsers);
    });
    channel.subscribe(async (status) => {
      if (status === 'SUBSCRIBED' && isMounted) {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) await channel.track({ user: { id: user.id, name: user.user_metadata?.full_name || user.email?.split('@')[0], email: user.email } });
      }
    });
      ...
      try {
        ...
        const normalized = (data || []).map((h: any) => ({
          ...
          durations: (h.durations || []).map((d: any) => ({
            ...d, hotelId: d.hotel_id, startDate: d.start_date, endDate: d.end_date, roomCards: (d.room_cards || []).map((rc: any) => ({
              ...rc, durationId: rc.duration_id, employees: (rc.employees || []).map((e: any) => ({ ...e, checkIn: e.checkin, checkOut: e.checkout }))
            }))
          }))
        }));
        if (isMounted) { setSystemCompanies(companies); setHotels(normalized); setHistory([normalized]); setHistoryIndex(0); }
      } catch (err: any) { setError(err.message); } finally { if (isMounted) setLoading(false); }
    }
    fetchAllData();
  }, [selectedYear]);
  ...
  const handleAddGlobalCompany = async (name: string) => { await addSystemCompany(name); setSystemCompanies(prev => [...new Set([...prev, name])]); };
  const handleDeleteGlobalCompany = async (name: string) => { await deleteSystemCompany(name); setSystemCompanies(prev => prev.filter(c => c !== name)); };
  ...
  const finalFiltered = useMemo(() => {
    let result = hotels.filter(h => {
      ...
      if (searchQuery) {
          ...
          else if (searchScope === 'invoice') { if (!(h.rechnungNr?.toLowerCase().includes(q) || h.durations?.some((d:any) => d.rechnungNr?.toLowerCase().includes(q)))) return false; }
          else if (searchScope === 'employee') { if (!h.durations?.some((d:any) => d.roomCards?.some((rc:any) => rc.employees?.some((e:any) => e.name?.toLowerCase().includes(q))))) return false; }
    ...
    return result.sort((a, b) => {
      ...
      if (sortBy === 'bed_price') {
          const getP = (h: any) => {
            if (h.override_price_per_bed > 0) return parseFloat(h.override_price_per_bed);
            let min = Infinity;
            h.durations?.forEach((d:any) => d.roomCards?.forEach((c:any) => {
                ...
                if (netto > 0 && beds > 0 && nights > 0) { const p = netto / (beds * nights); if (p < min) min = p; }
            }));
  ...
  const groupData = useMemo(() => {
    ...
    finalFiltered.forEach(h => {
      const key = groupBy === 'company' ? (h.companyTag?.[0] || (lang === 'de' ? 'Nicht zugeordnet' : 'Unassigned')) : groupBy === 'city' ? (h.city || 'Other') : h.name;
  ...
  const activeFilters = useMemo(() => {
    ...
    if (tlType !== 'all') badges.push({ id: 'tl', label: lang === 'de' ? 'Zeitraum' : 'Timeline', val: tlType === 'range' ? `${fmt(tlStart)} ➔ ${fmt(tlEnd)}` : tlType, clear: () => setTlType('all') });
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <div className={cn('px-8 py-5 border-b shrink-0 z-40 relative', dk ? 'bg-[#0F172A] border-white/5' : 'bg-white border-slate-200')}>
          <div className="flex items-center justify-between flex-wrap gap-4 w-full">
            ...
            {activeUsers.length > 0 && (<div className="flex items-center gap-3"><span className="text-[10px] font-bold uppercase tracking-widest opacity-50">{lang === 'de' ? 'Live dabei:' : 'Live now:'}</span><div className="flex -space-x-3">{activeUsers.map((u: any) => (<div key={u.id} className="relative group cursor-pointer"><div className="w-10 h-10 rounded-full border-2 border-white bg-teal-600 flex items-center justify-center text-white text-sm font-bold shadow-md">{u.name.substring(0, 2).toUpperCase()}</div></div>))}</div></div>)}
          </div>
        </div>
        ...
        <main className="flex-1 overflow-y-auto p-8 relative no-scrollbar pb-64">
            <div className="flex items-center justify-between mb-4 flex-wrap z-[100] relative">
              ...
              <div className="flex items-center gap-2">
                    ...
                    <div className={popupCls}>
                      ...
                      <div className="grid grid-cols-4 gap-2 mb-4">
                        {['today', 'tomorrow', '3days', '7days'].map(t => (
                          <button key={t} onClick={() => setTlType(t as any)} className={cn("py-2 rounded-lg text-xs font-medium border", tlType === t ? btnActive : btnInactive)}>{lang === 'de' ? (t==='today'?'Heute':t==='tomorrow'?'Morgen':t==='3days'?'3 Tage':'7 Tage') : t}</button>
                        ))}
                      </div>
                    ...
                    <div className={popupCls}>
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-6">
                        {[{id:'name', lEn:'Hotel Name', lDe:'Hotelname'}, {id:'cost', lEn:'Total Cost', lDe:'Gesamtkosten'}, {id:'bed_price', lEn:'Price/Bed', lDe:'Preis/Bett'}, {id:'free_beds', lEn:'Free Beds', lDe:'Freie Betten'}].map(s => (
                          <button key={s.id} onClick={() => setSortBy(s.id as any)} className={cn("py-3 rounded-lg text-sm font-medium border", sortBy === s.id ? btnActive : btnInactive)}>{lang === 'de' ? s.lDe : s.lEn}</button>
                        ))}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-6">
                        <button onClick={() => setSortDir('asc')} className={cn("py-3 px-4 rounded-lg border text-left", sortDir === 'asc' ? btnActive : btnInactive)}>
                          ...
                          <span className="text-[10px] opacity-60">{lang === 'de' ? 'Aufsteigend, A-Z, Günstigste' : 'Low to High, A-Z, Oldest'}</span>
                        </button>
                        <button onClick={() => setSortDir('desc')} className={cn("py-3 px-4 rounded-lg border text-left", sortDir === 'desc' ? btnActive : btnInactive)}>
                          ...
                          <span className="text-[10px] opacity-60">{lang === 'de' ? 'Absteigend, Z-A, Neueste' : 'High to Low, Z-A, Newest'}</span>
                        </button>
                      </div>
                    </div>
                  )}
                ...
                <button onClick={() => setAddingHotel(true)} className="ml-4 px-6 py-2.5 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl flex items-center gap-2 text-sm transition-all shadow-md active:scale-95"><Plus size={18} strokeWidth={2.5} /> {lang === 'de' ? 'Hotel hinzufügen' : 'Add Hotel'}</button>
              </div>
            </div>
            ...
            {groupBy !== 'none' && groupData && (
              <div className="flex gap-2 mb-6 overflow-x-auto pb-2 no-scrollbar border-b border-slate-200 dark:border-white/10">
                {Object.keys(groupData).map(g => (
                  <button key={g} onClick={() => setActiveGroupTab(g === activeGroupTab ? null : g)} className={cn("px-5 py-2.5 rounded-t-xl text-sm font-bold border transition-all whitespace-nowrap", activeGroupTab === g ? "bg-teal-600 text-white border-teal-600" : "bg-white dark:bg-white/5 text-slate-500 border-transparent hover:bg-slate-100")}>
              ...
              <div className="flex flex-wrap items-center gap-2 mb-6">
                ...
                {activeFilters.map(af => (
                  <span key={af.id} className={cn("px-3 py-1.5 rounded-lg flex items-center gap-2 text-[11px] font-bold border", dk ? "bg-teal-500/20 text-teal-400 border-teal-500/30" : "bg-teal-50 border-teal-200 text-teal-700")}>
                    ...
                    <button onClick={af.clear} className="hover:text-red-500 transition-colors"><X size={12} strokeWidth={3} /></button>
                  </span>
                ))}
                <button onClick={() => { setFilterPaid('all'); setGroupBy('none'); setSortBy('created_at'); setSortDir('desc'); }} className="text-xs font-bold text-slate-400 hover:underline ml-2">{lang === 'de' ? 'Alle löschen' : 'Clear All'}</button>
              </div>
            )}

            <div className="flex flex-col gap-6">
                {addingHotel && (<div className="rounded-2xl border p-5 bg-white dark:bg-[#1E293B] shadow-xl"><div className="flex gap-4 items-end"><div className="flex-1"><label className="text-[10px] uppercase font-bold text-slate-500 mb-1.5 block">{lang === 'de' ? 'Hotelname' : 'Hotel Name'}</label><input autoFocus value={newHotelName} onChange={e => setNewHotelName(e.target.value)} className="w-full h-10 px-3 rounded-lg border dark:bg-black/20" /></div><button onClick={handleSaveNewHotel} className="h-10 px-6 bg-teal-600 text-white font-bold rounded-lg">Save</button><button onClick={() => setAddingHotel(false)}><X/></button></div></div>)}
