...
export default function Dashboard({ theme, lang, toggleTheme, setLang, viewOnly = false, accessLevel, onSignOut, offlineMode, onToggleOfflineMode }: DashboardProps) {
  ...
  // Calculate dynamic free beds for UI buttons
    ...
    <div className={cn('flex h-screen overflow-hidden', dk ? 'bg-[#0F172A]' : 'bg-slate-50')}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto p-8 relative no-scrollbar pb-64">
          <div className="flex items-center justify-between mb-8 gap-4 flex-wrap relative z-[100]">
            ...
            <div className="flex items-center gap-2">
                  ...
                  <div className={cn(popupCls, 'right-0 w-[400px]')}>
                    ...
                    <div className={cn("p-4 rounded-xl border mb-4", dk ? "bg-black/20 border-white/5" : "bg-slate-50 border-slate-200")}>
                      <div className="flex items-center gap-3">
                         <input type="date" value={tlStart} onChange={e => {setTlStart(e.target.value); setTlType('range');}} className={cn("flex-1 p-2 rounded-lg outline-none text-sm", dk ? "bg-transparent border border-white/10" : "bg-white border border-slate-200")} />
                         <span className="text-slate-400">➔</span>
                         <input type="date" value={tlEnd} onChange={e => {setTlEnd(e.target.value); setTlType('range');}} className={cn("flex-1 p-2 rounded-lg outline-none text-sm", dk ? "bg-transparent border border-white/10" : "bg-white border border-slate-200")} />
                      </div>
                    </div>
                       ...
                       <div>
                         ...
                         <div className="grid grid-cols-2 gap-2 mb-2">
                           {[ { id: 'today', lEn: `Today (${fbCountToday})`, lDe: `Heute (${fbCountToday})` }, { id: 'tomorrow', lEn: `Tomorrow (${fbCountTomorrow})`, lDe: `Morgen (${fbCountTomorrow})` }, { id: '3days', lEn: `in 3 days (${fbCount3})`, lDe: `in 3 Tagen (${fbCount3})` }, { id: '7days', lEn: `in 7 days (${fbCount7})`, lDe: `in 7 Tagen (${fbCount7})` }
                             ...
                             <button key={f.id} onClick={() => setFbType(f.id as any)} className={cn("py-2 rounded-lg text-sm font-medium transition-all", fbType === f.id ? btnActive : btnInactive)}>
                               {lang === 'de' ? f.lDe : f.lEn}
                             </button>
                           ))}
                         </div>
              ...
              {/* SORT MENU */}
              <div className="relative">
                <button onClick={() => { setShowTimelineMenu(false); setShowFilterMenu(false); setShowSortMenu(!showSortMenu); }} className={cn("px-4 py-2.5 rounded-xl border text-sm font-medium flex items-center gap-2", showSortMenu ? btnActive : btnInactive)}><ArrowUpDown size={16} /> {lang === 'de' ? 'Sortieren' : 'Sort'}</button>
                {showSortMenu && (
                  <div className={cn(popupCls, 'w-[420px]')}>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                      {[
                        { id: 'name', lEn: 'Hotel Name', lDe: 'Hotelname' }, { id: 'cost', lEn: 'Total Cost', lDe: 'Gesamtkosten' },
                        { id: 'bed_price', lEn: 'Price/Bed', lDe: 'Preis/Bett' }, { id: 'free_beds', lEn: 'Free Beds', lDe: 'Freie Betten' },
                        { id: 'created_at', lEn: 'Last Added', lDe: 'Zuletzt Hinzugefügt' }, { id: 'updated_at', lEn: 'Last Updated', lDe: 'Zuletzt Aktualisiert' }
                      ].map(s => (
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                      <button onClick={() => setSortDir('asc')} className={cn("py-3 rounded-lg border text-left px-4 transition-all", sortDir === 'asc' ? btnActive : btnInactive)}>
                         <span className="block text-sm font-medium">{lang === 'de' ? 'Aufsteigend' : 'Ascending'}</span>
                         <span className={cn("block text-[10px] mt-1 font-normal", sortDir === 'asc' ? 'opacity-90' : 'opacity-50')}>{lang === 'de' ? 'Aufsteigend, A-Z, Älteste' : 'Low to High, A-Z, Oldest'}</span>
                      </button>
                      <button onClick={() => setSortDir('desc')} className={cn("py-3 rounded-lg border text-left px-4 transition-all", sortDir === 'desc' ? btnActive : btnInactive)}>
                         <span className="block text-sm font-medium">{lang === 'de' ? 'Absteigend' : 'Descending'}</span>
                         <span className={cn("block text-[10px] mt-1 font-normal", sortDir === 'desc' ? 'opacity-90' : 'opacity-50')}>{lang === 'de' ? 'Absteigend, Z-A, Neueste' : 'High to Low, Z-A, Newest'}</span>
                      </button>
                    </div>
              ...
              <button onClick={() => setShowBookmarks(!showBookmarks)} className={cn("px-4 py-2.5 rounded-xl border text-sm font-medium flex items-center gap-2", showBookmarks ? "bg-yellow-500 text-white border-transparent" : btnInactive)}><Star size={16} className={showBookmarks ? 'fill-white' : ''} /> {lang === 'de' ? 'Lesezeichen' : 'Bookmarks'}</button>
              
              {!isStrictViewer && (
                <button onClick={() => setAddingHotel(true)} className="ml-4 px-6 py-2.5 bg-[#0D9488] hover:bg-[#0f766e] text-white font-bold rounded-xl flex items-center gap-2 text-sm transition-all shadow-md">
              ...
              {addingHotel && !isStrictViewer && (
                <div className={cn('rounded-2xl border p-6 shadow-xl mb-4 animate-in slide-in-from-top duration-300', dk ? 'bg-[#1E293B] border-teal-500/30' : 'bg-white border-teal-500/30')}>
                  <div className="flex flex-wrap lg:flex-nowrap gap-4 items-end">
                    <div className="flex-1 min-w-[200px]"><label className="text-xs font-bold text-slate-500 mb-1 block">{lang === 'de' ? 'Hotelname' : 'Hotel Name'}</label><input autoFocus className={cn('w-full px-4 py-2.5 rounded-lg border outline-none text-sm transition-all focus:border-teal-500', dk ? 'bg-black/20 border-white/10 text-white' : 'bg-slate-50 border-slate-200')} value={newHotelName} onChange={e => setNewHotelName(e.target.value)} placeholder="Riveria..." /></div>
                    <div className="w-48"><label className="text-xs font-bold text-slate-500 mb-1 block"><MapPin size={12} className="inline mr-1"/> {lang === 'de' ? 'Stadt' : 'City'}</label><input className={cn('w-full px-4 py-2.5 rounded-lg border outline-none text-sm transition-all focus:border-teal-500', dk ? 'bg-black/20 border-white/10 text-white' : 'bg-slate-50 border-slate-200')} value={newHotelCity} onChange={e => setNewHotelCity(e.target.value)} placeholder="Essen..." /></div>
                ...
                Object.entries(groupData).map(([gName, hList]) => (
                  <div key={gName} className="space-y-4">
                     <div className={cn("px-6 py-4 rounded-xl border flex items-center justify-between", dk ? "bg-black/20 border-white/10" : "bg-white border-slate-200 shadow-sm")}>
                        ...
                        <div className="text-right"><p className="text-[10px] font-bold text-slate-500 uppercase">{lang === 'de' ? 'Gesamtwert' : 'Total Value'}</p><p className="text-lg font-bold text-teal-600">{formatCurrency(hList.reduce((s,h)=>s+calcHotelTotalCost(h),0))}</p></div>
                     </div>
