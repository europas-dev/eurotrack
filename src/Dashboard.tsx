...
export default function Dashboard({ isDarkMode: dk, lang = 'de', selectedMonth, selectedYear }: Props) {
  ...
  const inputBase = cn(
    'px-3 py-1.5 rounded-lg border text-sm outline-none transition-all',
  ...
  const btn = cn(
    'px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all flex items-center gap-1.5 shrink-0',
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      {/* ══ STICKY TOOLBAR ══════════════════════════════════════════════════ */}
      <div className={cn(
        'sticky top-0 z-20 px-4 py-2 border-b flex items-center gap-2 flex-wrap',
          ...
          {showFilters && (
            <div className={cn('absolute top-full left-0 mt-2 w-80 rounded-xl border shadow-2xl p-4 space-y-4 z-40',
              ...
              <div className="flex items-center justify-between">
                <p className={cn('text-xs font-bold uppercase tracking-wide', dk ? 'text-slate-400' : 'text-slate-500')}>
              ...
              <div className="space-y-2">
                <p className={cn('text-[10px] font-bold uppercase tracking-wide', dk ? 'text-slate-500' : 'text-slate-400')}>
                ...
                {freeBedFilter === 'custom' && (
                  <input type="date" value={freeBedCustomDate} onChange={e => setFreeBedCustomDate(e.target.value)} className={cn(inputBase, 'w-full')} />
                )}
              </div>
        ...
        <div className="flex items-center gap-2 ml-auto">
          <select value={sortBy} onChange={e => setSortBy(e.target.value as SortBy)} className={cn(inputBase, 'cursor-pointer')}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
        {loading ? (
          ...
          Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([group, items]) => (
            <div key={group}>
              <p className={cn('text-[10px] font-bold uppercase tracking-widest px-1 pb-1 pt-2', dk ? 'text-slate-500' : 'text-slate-400')}>
