...
import { Plus, Check, X, Loader2, Filter, ArrowUpDown, Undo2, Redo2, Star, Calendar, MapPin, Building, Building2, CloudOff, Globe, Trash2, Search } from 'lucide-react';
import Header from './components/Header';
import Sidebar from './components/Sidebar';
import { HotelRow, ModernDropdown, CompanyMultiSelect, getCountryOptions } from './components/HotelRow';
import ExportStudio from './components/ExportStudio';

// --- SYSTEM COMPANIES API stays the same ---
...
export default function Dashboard({ theme, lang, toggleTheme, setLang, viewOnly = false, accessLevel, onSignOut, offlineMode, onToggleOfflineMode }: DashboardProps) {
  ...
  useEffect(() => { 
    ...
    async function fetchAllData() {
      ...
      } catch (err: any) { if (isMounted) setError(err.message || "Failed to load dashboard data."); } finally { clearTimeout(safetyTimer); if (isMounted) setLoading(false); }
    }
    fetchAllData(); 
    return () => { isMounted = false; clearTimeout(safetyTimer); };
  }, [selectedYear]);

  const allCompanyOptions = useMemo(() => { const localTags = hotels.flatMap(h => h.companyTag || []).filter(Boolean); return Array.from(new Set([...systemCompanies, ...localTags])); }, [hotels, systemCompanies]);
  const uniqueCities = useMemo(() => Array.from(new Set(hotels.map(h => h.city).filter(Boolean))), [hotels]);

  const handleAddGlobalCompany = async (name: string) => { try { await addSystemCompany(name); setSystemCompanies(prev => Array.from(new Set([...prev, name]))); } catch (err) { console.error(err); } };
  const handleDeleteGlobalCompany = async (name: string) => { try { await deleteSystemCompany(name); setSystemCompanies(prev => prev.filter(c => c !== name)); } catch (err) { console.error(err); } };
  ...
  const finalFiltered = useMemo(() => {
    return hotels.filter(h => {
      ...
      // --- SURGICAL FIX: SMART TARGETED SEARCH LOGIC ---
      if (searchQuery) {
  ...
  return (
    <div className={cn('flex h-screen overflow-hidden', dk ? 'bg-[#0F172A]' : 'bg-slate-50')}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <Header theme={theme} lang={lang} toggleTheme={toggleTheme} setLang={setLang} searchQuery={searchQuery} setSearchQuery={setSearchQuery} searchScope={searchScope} setSearchScope={setSearchScope} onSignOut={onSignOut} onExportCsv={() => exportToCSV(finalFiltered, calcHotelTotalCost, totalSpend, `Report: Period ${displayTitle}`, lang, filterPaid !== 'all', filterDeposit !== 'all')} onPrint={() => setShowStudio(true)} viewOnly={isStrictViewer} userRole={accessLevel?.role ?? 'viewer'} offlineMode={offlineMode} onToggleOfflineMode={onToggleOfflineMode} isOnline={isOnline} />
        {(!isOnline || offlineMode) && (<div className="bg-amber-500 border-b border-amber-600 text-white px-6 py-2.5 text-sm font-bold flex items-center justify-center gap-2 z-[60] relative"><CloudOff size={16} strokeWidth={2.5} /> {lang === 'de' ? 'Offline Modus Aktiv' : 'Offline Mode Active'}</div>)}
        <div className={cn('px-8 py-5 border-b shrink-0 z-40 relative', dk ? 'bg-[#0F172A] border-white/5' : 'bg-white border-slate-200')}>
          <div className="flex items-center justify-between flex-wrap gap-4 w-full">
            ...
            {activeUsers.length > 0 && (<div className="flex items-center gap-3"><span className="text-[10px] font-bold uppercase tracking-widest opacity-50">{lang === 'de' ? 'Live dabei:' : 'Live now:'}</span><div className="flex -space-x-3">{activeUsers.map((u: any, i: number) => (<div key={i} className="relative group cursor-pointer"><div className={cn("w-10 h-10 rounded-full border-2 flex items-center justify-center text-white text-sm font-bold shadow-md z-10 relative", dk ? "bg-teal-600 border-[#0F172A]" : "bg-teal-600 border-white")}>{u.name.substring(0, 2).toUpperCase()}</div><div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-max px-3 py-2 bg-slate-800 text-white text-xs rounded-xl opacity-0 group-hover:opacity-100 transition-opacity z-[100] pointer-events-none shadow-xl">{u.name} <br/> <span className="text-slate-400 text-[10px]">{u.email}</span></div></div>))}</div></div>)}
          </div>
        </div>
        {error && (<div className="bg-red-500 text-white px-6 py-2 text-sm font-bold flex items-center justify-center gap-2 z-[60] relative">Error: {error}<button onClick={() => setError('')} className="ml-4 underline hover:text-red-200">Dismiss</button></div>)}
        {loading ? (<div className="flex-1 flex items-center justify-center"><Loader2 size={48} className="animate-spin text-teal-500 opacity-50" /></div>) : (
          <main className="flex-1 overflow-y-auto p-8 relative no-scrollbar pb-64">
            <div className="flex items-center justify-between mb-4 gap-4 flex-wrap relative z-[100]">
              ...
              <div className="flex items-center gap-2">
                <div className={cn("flex items-center mr-2 rounded-xl p-1 border", dk ? "bg-[#1E293B] border-white/10" : "bg-white border-slate-200 shadow-sm")}><button onClick={handleUndo} disabled={historyIndex <= 0} className="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 disabled:opacity-30 transition-all" title="Undo"><Undo2 size={18}/></button><button onClick={handleRedo} disabled={historyIndex >= history.length - 1} className="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 disabled:opacity-30 transition-all" title="Redo"><Redo2 size={18}/></button></div>
                <div className="relative">
                  ...
                  {showTimelineMenu && (<div className={cn(popupCls, 'right-0 w-[400px]')}><div className="flex items-center justify-between mb-5"><h3 className="text-lg font-bold">{lang === 'de' ? 'Zeitraum' : 'Timeline Card (Stay Overlap)'}</h3><button onClick={closeMenu} className="text-slate-400 hover:text-white"><X size={20}/></button></div><div className="grid grid-cols-4 gap-2 mb-4">{[{ id: 'today', lEn: 'Today', lDe: 'Heute', off: 0 }, { id: 'tomorrow', lEn: 'Tomorrow', lDe: 'Morgen', off: 1 }, { id: '3days', lEn: '3 Days', lDe: '3 Tage', off: 3 }, { id: '7days', lEn: '7 Days', lDe: '7 Tage', off: 7 }].map(t => (<button key={t.id} onClick={() => { const s = new Date(); if (t.off > 0 && t.id !== '3days' && t.id !== '7days') s.setDate(s.getDate() + t.off); const e = new Date(); e.setDate(e.getDate() + t.off); setTlStart(s.toISOString().split('T')[0]); setTlEnd(e.toISOString().split('T')[0]); setTlType(t.id as any); }} className={cn("py-2 rounded-lg text-xs font-medium border transition-all", tlType === t.id ? btnActive : btnInactive)}>{lang === 'de' ? t.lDe : t.lEn}</button>))}</div><div className={cn("p-4 rounded-xl border mb-4", dk ? "bg-black/20 border-white/5" : "bg-slate-50 border-slate-200")}><div className="flex items-center gap-3"><input type="date" value={tlStart} onChange={e => {setTlStart(e.target.value); setTlType('range');}} className={cn("flex-1 p-2 rounded-lg outline-none text-sm font-medium", dk ? "bg-transparent border border-white/10" : "bg-white border border-slate-200")} /><span className="text-slate-400">➔</span><input type="date" min={tlStart} value={tlEnd} onChange={e => {if(tlStart && e.target.value < tlStart) return; setTlEnd(e.target.value); setTlType('range');}} className={cn("flex-1 p-2 rounded-lg outline-none text-sm font-medium", dk ? "bg-transparent border border-white/10" : "bg-white border border-slate-200")} /></div></div><p className="text-xs text-slate-500 mb-6">{lang === 'de' ? 'Blendet alle Hotels ohne Buchungen im gewählten Zeitraum aus.' : 'Hides any hotel that has zero bookings/durations overlapping your chosen range.'}</p><div className="flex justify-center border-t border-slate-200 dark:border-white/10 pt-4"><button onClick={() => {setTlType('all'); setTlStart(''); setTlEnd(''); closeMenu();}} className={cn("w-full py-2.5 rounded-lg border flex items-center justify-center gap-2 text-sm font-medium", dk ? "border-teal-600 text-teal-500 hover:bg-teal-600/10" : "border-teal-600 text-teal-700 hover:bg-teal-50")}><Trash2 size={16}/> {lang === 'de' ? 'Zeitraum zurücksetzen' : 'Clear Timeline'}</button></div></div>)}
                </div>
                <div className="relative">
                  ...
                  {showFilterMenu && (<div className={popupCls}><div className="flex items-center justify-between mb-5"><h3 className="text-lg font-bold">Filter</h3><button onClick={closeMenu} className="text-slate-400"><X size={20}/></button></div><div className="space-y-5"><div><p className="text-sm text-slate-400 mb-2">{lang === 'de' ? 'Verfügbare freie Betten' : 'Free Beds Availability'}</p><div className="grid grid-cols-2 gap-2 mb-2">{[{ id: 'today', lEn: `Today (${fbCountToday})`, lDe: `Heute (${fbCountToday})` }, { id: 'tomorrow', lEn: `Tomorrow (${fbCountTomorrow})`, lDe: `Morgen (${fbCountTomorrow})` }, { id: '3days', lEn: `in 3 days (${fbCount3})`, lDe: `in 3 Tagen (${fbCount3})` }, { id: '7days', lEn: `in 7 days (${fbCount7})`, lDe: `in 7 Tagen (${fbCount7})` }].map(f => <button key={f.id} onClick={() => setFbType(f.id as any)} className={cn("py-2 rounded-lg text-sm font-medium transition-all border", fbType === f.id ? btnActive : btnInactive)}>{lang === 'de' ? f.lDe : f.lEn}</button>)}</div></div><div><p className="text-sm text-slate-400 mb-2">{lang === 'de' ? 'Zahlung' : 'Payment'}</p><div className="flex p-1 rounded-xl bg-slate-100 dark:bg-black/20">{[{id:'all', lEn:'All', lDe:'Alle'}, {id:'paid', lEn:'Paid', lDe:'Bezahlt'}, {id:'unpaid', lEn:'Unpaid', lDe:'Unbezahlt'}].map(p => <button key={p.id} onClick={() => setFilterPaid(p.id as any)} className={cn("flex-1 py-1.5 text-sm font-medium rounded-lg transition-all", filterPaid === p.id ? (dk ? "bg-teal-600 text-white shadow-sm" : "bg-white text-teal-700 shadow-sm") : "text-slate-500")}>{lang === 'de' ? p.lDe : p.lEn}</button>)}</div></div><div><p className="text-sm text-slate-400 mb-2">{lang === 'de' ? 'Kaution' : 'Deposit'}</p><div className="flex p-1 rounded-xl bg-slate-100 dark:bg-black/20">{[{id:'all', lEn:'All', lDe:'Alle'}, {id:'yes', lEn:'Yes', lDe:'Ja'}, {id:'no', lEn:'No', lDe:'Nein'}].map(d => <button key={d.id} onClick={() => setFilterDeposit(d.id as any)} className={cn("flex-1 py-1.5 text-sm font-medium rounded-lg transition-all", filterDeposit === d.id ? (dk ? "bg-teal-600 text-white shadow-sm" : "bg-white text-teal-700 shadow-sm") : "text-slate-500")}>{lang === 'de' ? d.lDe : d.lEn}</button>)}</div></div><div><p className="text-sm text-slate-400 mb-2">{lang === 'de' ? 'Gruppieren nach' : 'Group by'}</p><div className="flex p-1 rounded-xl bg-slate-100 dark:bg-black/20">{[{id:'none', lEn:'None', lDe:'Keine'}, {id:'hotel', lEn:'Hotel', lDe:'Hotel'}, {id:'company', lEn:'Company', lDe:'Firma'}, {id:'city', lEn:'City', lDe:'Stadt'}].map(g => <button key={g.id} onClick={() => setGroupBy(g.id as any)} className={cn("flex-1 py-1.5 text-sm font-medium rounded-lg transition-all", groupBy === g.id ? (dk ? "bg-teal-600 text-white shadow-sm" : "bg-white text-teal-700 shadow-sm") : "text-slate-500")}>{lang === 'de' ? g.lDe : g.lEn}</button>)}</div></div></div><div className="flex items-center justify-between border-t border-slate-200 dark:border-white/10 mt-6 pt-4"><button onClick={() => { setFilterPaid('all'); setFilterDeposit('all'); setGroupBy('none'); setFbType('all'); }} className={actionSecondary}>{lang === 'de' ? 'Alle Filter löschen' : 'Clear All filters'}</button><button onClick={closeMenu} className={actionPrimary}>{lang === 'de' ? 'Filter anwenden' : 'Apply Filters'}</button></div></div>)}
                </div>
                <div className="relative">
                  ...
                  {showSortMenu && (<div className={cn(popupCls, 'w-[420px]')}><div className="flex items-center justify-between mb-5"><h3 className="text-lg font-bold">{lang === 'de' ? 'Sortieren' : 'Sort'}</h3><button onClick={closeMenu} className="text-slate-400"><X size={20}/></button></div><p className="text-sm text-slate-400 mb-2">{lang === 'de' ? 'Sortieren nach' : 'Sort By'}</p><div className="grid grid-cols-2 gap-3 mb-6">{[{ id: 'name', lEn: 'Hotel Name', lDe: 'Hotelname' }, { id: 'cost', lEn: 'Total Cost', lDe: 'Gesamtkosten' }, { id: 'bed_price', lEn: 'Price/Bed', lDe: 'Preis/Bett' }, { id: 'free_beds', lEn: 'Free Beds', lDe: 'Freie Betten' }, { id: 'created_at', lEn: 'Last Added', lDe: 'Zuletzt Hinzugefügt' }, { id: 'updated_at', lEn: 'Last Updated', lDe: 'Zuletzt Aktualisiert' }].map(s => <button key={s.id} onClick={() => setSortBy(s.id as any)} className={cn("py-3 rounded-lg text-sm font-medium border transition-all", sortBy === s.id ? btnActive : btnInactive)}>{lang === 'de' ? s.lDe : s.lEn}</button>)}</div><p className="text-sm text-slate-400 mb-2">{lang === 'de' ? 'Sortierreihenfolge' : 'Sort Direction'}</p><div className="grid grid-cols-2 gap-3 mb-6"><button onClick={() => setSortDir('asc')} className={cn("py-3 rounded-lg border text-left px-4 transition-all", sortDir === 'asc' ? btnActive : btnInactive)}><span className="block text-sm font-medium">{lang === 'de' ? 'Aufsteigend' : 'Ascending'}</span><span className={cn("block text-[10px] mt-1 font-normal", sortDir === 'asc' ? 'opacity-90' : 'opacity-50')}>{lang === 'de' ? 'Aufsteigend' : 'Low to High'}</span></button><button onClick={() => setSortDir('desc')} className={cn("py-3 rounded-lg border text-left px-4 transition-all", sortDir === 'desc' ? btnActive : btnInactive)}><span className="block text-sm font-medium">{lang === 'de' ? 'Absteigend' : 'Descending'}</span><span className={cn("block text-[10px] mt-1 font-normal", sortDir === 'desc' ? 'opacity-90' : 'opacity-50')}>{lang === 'de' ? 'Absteigend' : 'High to Low'}</span></button></div><div className="flex items-center justify-between border-t border-slate-200 dark:border-white/10 pt-4"><button onClick={() => { setSortBy('created_at'); setSortDir('desc'); }} className={actionSecondary}>{lang === 'de' ? 'Sortierung zurücksetzen' : 'Reset Sorting'}</button><button onClick={closeMenu} className={actionPrimary}>{lang === 'de' ? 'Sortierung anwenden' : 'Apply Sorting'}</button></div></div>)}
                </div>
                ...
                {!isStrictViewer && (<button onClick={() => setAddingHotel(true)} className="ml-4 px-6 py-2.5 bg-[#0D9488] hover:bg-[#0f766e] text-white font-bold rounded-xl flex items-center gap-2 text-sm transition-all shadow-md active:scale-95"><Plus size={18} strokeWidth={2.5} /> {lang === 'de' ? 'Hotel hinzufügen' : 'Add Hotel'}</button>)}
              </div>
            </div>
            {activeFilters.length > 0 && (<div className="flex flex-wrap items-center gap-2 mb-6 animate-in fade-in duration-200"><span className="text-xs font-bold text-slate-500 mr-2">{lang === 'de' ? 'Aktive Filter:' : 'Active Filters:'}</span>{activeFilters.map(af => (<span key={af.id} className={cn("px-3 py-1.5 rounded-lg flex items-center gap-2 text-[11px] font-bold border", dk ? "bg-teal-500/20 text-teal-400 border-teal-500/30" : "bg-teal-50 border-teal-200 text-teal-700")}>{af.label}: <span className="opacity-70 uppercase">{af.val}</span><button onClick={af.clear} className="hover:text-red-500 ml-1 transition-colors"><X size={12} strokeWidth={3} /></button></span>))}<button onClick={() => { setTlType('all'); setFbType('all'); setFilterPaid('all'); setFilterDeposit('all'); setGroupBy('none'); setSortBy('created_at'); setSortDir('desc'); setShowBookmarks(false); }} className="text-xs font-bold text-slate-400 hover:text-slate-600 dark:hover:text-white ml-2 transition-all hover:underline">{lang === 'de' ? 'Alle löschen' : 'Clear All'}</button></div>)}
            <div className="flex flex-col gap-6">
                {addingHotel && !isStrictViewer && (<div className={cn('rounded-2xl border p-5 shadow-xl mb-4 animate-in slide-in-from-top duration-300', dk ? 'bg-[#1E293B] border-teal-500/30' : 'bg-white border-teal-500/30')}><div className="flex flex-wrap lg:flex-nowrap items-end gap-4 w-full"><div className="flex-1 min-w-[200px]"><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 block">{lang === 'de' ? 'Hotelname' : 'Hotel Name'}</label><input autoFocus className={cn('w-full h-[38px] px-3 rounded-lg border outline-none text-sm font-bold transition-all focus:border-teal-500', dk ? 'bg-[#0F172A] border-white/10 text-white' : 'bg-slate-50 border-slate-200')} value={newHotelName} onChange={e => setNewHotelName(e.target.value)} placeholder="Riveria..." /></div><div className="flex-[0.8] min-w-[140px]"><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 block"><MapPin size={10} className="inline mr-1"/> {lang === 'de' ? 'Stadt' : 'City'}</label><input className={cn('w-full h-[38px] px-3 rounded-lg border outline-none text-sm font-bold transition-all focus:border-teal-500', dk ? 'bg-[#0F172A] border-white/10 text-white' : 'bg-slate-50 border-slate-200')} value={newHotelCity} onChange={e => setNewHotelCity(e.target.value)} placeholder="Essen..." /></div><div className="flex-1 min-w-[160px]"><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 block"><Building2 size={10} className="inline mr-1"/> {lang === 'de' ? 'Firma' : 'Company'}</label><div className={cn('w-full min-h-[38px] rounded-lg border px-2 flex items-center transition-all', dk ? 'bg-[#0F172A] border-white/10 text-white' : 'bg-slate-50 border-slate-200')}><CompanyMultiSelect selected={newHotelCompanyTags} options={allCompanyOptions} onChange={(v: string[]) => setNewHotelCompanyTags(v)} isDarkMode={dk} lang={lang} onDeleteOption={handleDeleteGlobalCompany} onAddOption={handleAddGlobalCompany} /></div></div><div className="flex-[0.8] min-w-[120px]"><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 block"><Globe size={10} className="inline mr-1"/> {lang === 'de' ? 'Land' : 'Country'}</label><ModernDropdown value={newHotelCountry} options={getCountryOptions()} onChange={(v: string) => setNewHotelCountry(v)} isDarkMode={dk} lang={lang} /></div><div className="flex gap-2 shrink-0"><button onClick={handleSaveNewHotel} disabled={newHotelSaving || !newHotelName.trim()} className="px-5 h-[38px] bg-teal-600 hover:bg-teal-700 text-white rounded-lg shadow-sm disabled:opacity-50 font-bold">{newHotelSaving ? <Loader2 size={18} className="animate-spin" /> : <Check size={18} />}</button><button onClick={() => setAddingHotel(false)} className={cn("px-4 h-[38px] rounded-lg flex items-center justify-center transition-all border", dk ? "border-white/10 hover:bg-white/10 text-slate-300" : "border-slate-200 hover:bg-slate-100 text-slate-600")}><X size={18} /></button></div></div></div>)}
                {groupBy !== 'none' && groupData ? Object.entries(groupData).map(([gName, hList]) => (<div key={gName} className="space-y-4"><div className={cn("px-6 py-4 rounded-xl border flex items-center justify-between", dk ? "bg-black/20 border-white/10" : "bg-white border-slate-200 shadow-sm")}><div className="flex items-center gap-4"><span className="text-xs font-bold uppercase tracking-wider text-slate-500">{lang === 'de' ? 'Gruppe' : 'Group'}: {groupBy}</span><h3 className="text-xl font-bold">{gName}</h3><span className="px-3 py-1 rounded-full bg-teal-500/10 text-teal-600 text-xs font-bold">{hList.length} Hotels</span></div><div className="text-right"><p className="text-[10px] font-bold text-slate-500 uppercase">{lang === 'de' ? 'Gesamtwert' : 'Total Value'}</p><p className="text-lg font-bold text-teal-600 dark:text-teal-400">{formatCurrency(hList.reduce((s,h)=>s+calcHotelTotalCost(h),0))}</p></div></div><div className="flex flex-col gap-4 pl-4 border-l-2 border-teal-500/30">{hList.map((h, i) => <HotelRow key={h.id} entry={h} index={i} isDarkMode={dk} lang={lang} searchQuery={searchQuery} searchScope={searchScope} companyOptions={allCompanyOptions} cityOptions={uniqueCities} onDelete={hId => setHotels(hotels.filter(ho=>ho.id!==hId))} onUpdate={(hId, up) => setHotels(hotels.map(ho=>ho.id===hId?{...ho,...up}:ho))} onDeleteCompanyOption={handleDeleteGlobalCompany} onAddOption={handleAddGlobalCompany} />)}</div></div>)) : finalFiltered.map((hotel, index) => (
                  <HotelRow 
                    key={hotel.id} 
                    entry={hotel} 
                    ...
                    isDarkMode={dk} 
                    lang={lang} 
                    // SURGICAL FIX: Pass query and scope down so highlight only matches selection
                ...
                {!loading && finalFiltered.length === 0 && (<div className="text-center py-20 opacity-50 flex flex-col items-center"><Building size={48} className="mb-4 text-slate-400" /><p className="text-lg font-bold">{lang === 'de' ? 'Keine Hotels gefunden' : 'No hotels found'}</p><p className="text-sm">{lang === 'de' ? 'Versuchen Sie, Ihre Filter anzupassen oder ein neues hinzuzufügen.' : 'Try adjusting your filters or adding a new one.'}</p></div>)}
            </div>
          </main>
        )}
        {showStudio && (<ExportStudio hotels={finalFiltered} calcCost={calcHotelTotalCost} lang={lang} total={totalSpend} title={selectedMonth !== null ? `Period: ${lang === 'de' ? monthNamesDe[selectedMonth] : monthNamesEn[selectedMonth]} ${selectedYear}` : `Period: ${selectedYear}`} onClose={() => setShowStudio(false)} dk={dk} />)}
      </div>
    </div>
  );
}
